...
void complexAbs(const cv::Mat& m, cv::Mat& output)
{
    ...
    planes[1] = Mat::zeros(m.rows, m.cols, CV_32F);
    cv::merge(planes,2,output);
}
...
void run(FPM_Dataset * dataset)
{
   ...
   for (int16_t itr = 1; itr <= dataset->itrCount; itr++)
   {
     ...
     currImg->Objfcrop = dataset->objF(cv::Rect(currImg->cropXStart,currImg->cropYStart,dataset->Np,dataset->Np));
     complexMultiply(currImg->Objfcrop, dataset->pupil, currImg->ObjfcropP);
      
      dft(currImg->ObjfcropP, currImg->ObjcropP, DFT_INVERSE); // Real-space of object
      
      // Replace Amplitude portion
      currImg->Image.convertTo(tmpMat1,CV_32FC1);
      planes[0] = tmpMat1;
      planes[1] = Mat::zeros(dataset->Np,dataset->Np, CV_32F);
      ...
      complexDivide(tmpMat2, tmpMat3, tmpMat1); // Invert to make the next line a division
      dft(tmpMat1,currImg->Objfup);
      ...
      complexDivide(tmpMat2, pupilMax * (tmpMat3 + dataset->delta2), tmpMat1);
      
      // Update fourier space of object
      dataset->objF(cv::Rect(currImg->cropXStart,currImg->cropYStart,dataset->Np,dataset->Np)) = dataset->objF(cv::Rect(currImg->cropXStart,currImg->cropYStart,dataset->Np,dataset->Np)) + tmpMat1;
      ...
      dft(dataset->objF ,dataset->objCrop, DFT_INVERSE);
      split(dataset->objCrop, planes);                   // planes[0] = Re(DFT(I), planes[1] = Im(DFT(I))
